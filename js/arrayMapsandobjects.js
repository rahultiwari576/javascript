...
// we can also create an array using array construct 
...
//? Push --------------------
marks.push(99888); //it will pust the number 99 into the array of marks and it will include the number at the end of the array
console.log(marks);
...
let xx = ['harsh', ""].filter((element) => { return element.length !== 0 }).length
...
let b = ary.map(function test(x) { // we get all the values of array in x one by one
    return x * 2
...
document.write(person.favMovies);
...
console.log(funcObject.add(5, 5));
console.log(funcObject.sub(10, 5));